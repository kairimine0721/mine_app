...
void GeometryCollimatorLEHR::Construct(G4LogicalVolume* LocalWorld_Logical) {
...
//  AHG4Material* tungsten = mmanager->instantiate(matname);
  ...
  G4double Collimator_pz = 12.025*mm;
 ...
 // G4LogicalVolume* Collimator_Logical = new G4LogicalVolume(a[imax-1][jmax-1], tungsten, "Collimator_Logical");
  G4LogicalVolume* Collimator_Logical = new G4LogicalVolume(a[imax-1][jmax-1], lead, "Collimator_Logical");
  ...
  double s_x = 0.0*mm-6.0*pitch_x;
  double s_y = 0.0*mm-7.0*pitch_y;
  
  G4ThreeVector pos;
  
  for(int i=0;i<13;i++){ //8
    for(int j=0;j<15;j++){ //9 17
