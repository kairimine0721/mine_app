...
CdTeStripCameraMainGeometryRealWorld::CdTeStripCameraMainGeometryRealWorld(std::string colname, std::string detname, G4ThreeVector pos, double radius)
  : constructed(false) , World_Logical(0) , World_Physical(0) , m_CollimatorName(colname), m_DetectorName(detname), w_sphere_center(pos), w_radius(radius){
}
...
void CdTeStripCameraMainGeometryRealWorld::SetupGeometry() {
  ...
  G4LogicalVolume* CdTeStripCamera_Logical = CdTeStripCamera_Geometry->Construct(m_CollimatorName, m_DetectorName, w_sphere_center, w_radius);
